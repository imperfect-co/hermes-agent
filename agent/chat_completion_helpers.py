...
def _gemini_summary_thinking_config(model: str) -> Dict[str, Any]:
    ...
    """
    # rsplit already drops any directory/repo prefix (``models/gemini-3.5-flash``
    # and ``google/gemini-3.5-flash`` both normalize to ``gemini-3.5-flash``), so
    # no separate ``models/`` strip is needed. ``gemini-3.1`` already starts with
    # ``gemini-3``, so a single ``gemini-3`` prefix covers the whole 3.x line.
    ...
    elif normalized.startswith("gemini-3"):  # gemini-3 pro
...
def handle_max_iterations(agent, messages: list, api_call_count: int) -> str:
    ...
    # Force extended thinking OFF across every summary path — OpenAI /
    # chat_completions, Anthropic Messages, and Codex Responses alike.
    # build_api_kwargs() / build_kwargs() read ``agent.reasoning_config`` when
    # they decide whether to emit a thinking/reasoning parameter, so swapping it
    # to ``{"enabled": False}`` here disables reasoning for ANY request built
    # during summary generation regardless of api_mode. A wrap-up summary must
    # spend its output budget on the summary text, not on reasoning (Gemini 3
    # dynamic thinking in particular would otherwise burn the whole budget and
    # return empty content). Restored in the ``finally`` so the agent's
    # configured reasoning is unchanged after the wrap-up. See issue #42.
    _original_reasoning_config = agent.reasoning_config
    ...
    try:
        ...
        # Strip any inline reasoning block the primary attempt returned. Reuse
        # the shared stripper (agent_runtime_helpers.strip_think_blocks) so the
        # summary path handles every reasoning-tag variant (<think>, <thinking>,
        # <thought>) plus orphaned tags the main storage boundary does. Call it
        # unconditionally on any non-empty text — a narrow "<think>" substring
        # guard would skip the other variants — and it is a no-op when clean.
        if final_response:
        ...
        # deterministic recap rather than a bare error — but keep an error
        # detail appended for debugging. Route the error through
        # _summarize_api_error so an attacker-influenced provider/proxy body
        # (which can echo Authorization / x-api-key / request JSON) is redacted
        ...
        final_response = (
            f"{_deterministic}\n\n"
            f"(Summary generation failed with error: {_error_detail})"
        )
        # Record the recap as the final assistant turn. The summary user-turn
        # appended at the top would otherwise dangle as the last message in
        # session history — a user turn with no assistant reply, which corrupts
        # replay on the next turn (Anthropic/Codex reject two trailing user
        # turns; the OpenAI path re-asks for a summary it already produced).
        messages.append({"role": "assistant", "content": final_response})
    finally:
        # Restore the caller's reasoning config regardless of success/failure so
        # the agent's configured thinking is unchanged after the wrap-up (#42).
        agent.reasoning_config = _original_reasoning_config
