...
DEFAULT_CONFIG = {
    ...
    "tts": {
        ...
        "gemini": {
            ...
            "voice": "Kore",
            # BCP-47 locale forced on the spoken output (e.g. "es-MX", "en-US").
            # Empty => Gemini infers the locale from the text. The voice-note
            # reply path (voice.tts_reply) auto-detects this per reply from the
            # inbound message, but a fixed value here pins it for the standalone
            # TTS tool.
            "language_code": "",
    ...
    "voice": {
        ...
        "auto_tts": False,
        # Voice-note replies (a.k.a. native audio output, ADR 0024). When True,
        # the gateway renders a spoken reply (Gemini TTS -> opus/ogg voice note)
        # whenever the inbound message was a voice note OR the user explicitly
        # asked for a spoken reply. Independent of auto_tts / the /voice toggle.
        # Render failures fall back to the plain text reply.
        #
        # Renamed from the legacy ``native_audio_out`` key, which is still read
        # for backwards compatibility when ``tts_reply`` is absent.
        "tts_reply": False,
        # Prebuilt Gemini voice used for voice-note replies.
